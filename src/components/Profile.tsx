...
export default function Profile() {
  ...
  return (
    <div className={`xs:px-0 md:px-52 min-h-screen bg-gradient-to-b ${whentoShowGameData ? 'brightness-75 ' : ''} from-white via-violet-200 to-white`}>
      <img src={profileheader} alt="profile image" className='w-full relative object-cover max-h-40 ' />

      <p className='absolute top-3 text-xs cursor-pointer font-manrope rounded-md font-semibold flex flex-row gap-2 items-center px-4 py-1 bg-violet-300 xs:left-10 md:left-60' onClick={() => navigate('/dashboard')}>
      ...
      <div>  
        {
          userData != undefined ?
          <p className='size-24 object-cover absolute top-28 rounded-full shadow bg-zinc-700 pt-6 font-manrope text-white text-4xl  text-center shadow-violet-200 border xs:left-7 md:left-[220px]'>{userData.username.toString().substring(0,1).toUpperCase()}</p>
          : 
          <img src={profile} alt="profile" className='size-24 object-cover absolute top-28 rounded-full shadow shadow-violet-200 border xs:left-7 md:left-[220px]'/>
        }
      </div>
          ...
          (
            <div>
              <div className='pt-[53px] md:grid md:grid-cols-2'>
                ...
                <div className='md:col-start-2 md:col-end-3 xs:px-6 md:px-0'>
              ...
              <div className='pt-8'>
                <p className='text-xs pb-1 font-manrope font-semibold xs:px-10 md:px-0'>Last 2 matches result :</p>
                <div className='grid grid-cols-6 gap-[1px] overflow-x-auto scrollbar-hide w-full'>
                    {userfieldOptions.map((data:userfieldOptionsType) => (
                        <p key={data?.id} className='text-center text-white py-1 font-manrope min-w-[40px] font-semibold text-xs bg-violet-500'>{data?.text}</p>
                    ))}
                </div>
                {
