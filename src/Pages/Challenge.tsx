...
import { useGameAccess } from '../hooks/useAccessHooks'
...
export default function Challenge() {
  ...
  const Add_Player_2_In_a_game = useCallback(async() => {
    try {
  ...
  const { checkIfGameIdValid,checkifGameisCompleted,checkingOtherValidationMessage,gamehasAlready_2Player,gamematchdata,player_2_hasnotJoinedyet,userData,warningMessage } = useGameAccess({gameid,ApiUrl,token,isLoggedIn,Add_Player_2_In_a_game})
 ...
 // const [userData,setUserData]=useState<UserDataType>()
  ...
  //     console.log('Wait Fetching Userdata and game data ...')
  ...
  //       if (userData?.data?.id !== gameData?.data?.player_1) {
  //         if (gameData?.data?.player_2_status === 'Player_2_Joined') {
  //           if (gameData?.data?.player_2 !== userData?.data?.id) {
  ...
  //     } catch (error) {
  //       console.error(`Issue Ouccred while making req: ${error}`);
  //     }
  //   }    
  ...
  //   setcheckingOtherValidationMessage(false)
  // },[isLoggedIn,userData,gamematchdata,token])
